#!/usr/bin/env python3
"""
Step 2: Powell Symmetric Identification.
This script refines the critical parameters of symmetric surge motion (m11, Xu, Xuu) 
using the Powell optimization method to minimize the simulation trajectory output error 
across training experiments, keeping sway and yaw parameters locked to Step 1 values.
"""
import os
import json
import argparse
...
def dynamic_from_vector(p):
    ...
    dyn["Xu"] = float(p[1])
    dyn["Xuu"] = float(p[2])
    dyn["model"] = "Fossen 3DOF 18-parameter output-error (Optimized Surge)"
    return dyn
...
def dynamic_cost(p, datasets, pool):
    ...
    if dynamic["m11"] < 20.0 or dynamic["Xu"] < 1.0 or dynamic["Xuu"] < 0.0:
        return 1e6
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Powell symmetric Identification.")
    ...
    print(f"\n--- OPTIMIZING DYNAMIC PARAMETERS ({num_cores} parallel cores) ---")
    p0_dynamic = np.array([
        DYNAMIC_START["m11"],
        DYNAMIC_START["Xu"],
        DYNAMIC_START["Xuu"]
    ...
    with open("identified_dynamics_powell.json", "w") as f:
        json.dump(dynamic, f, indent=4)
    with open("identified_motors_powell.json", "w") as f:
        json.dump(motor, f, indent=4)
    print("Final parameters saved as JSON!")

    param_names = [
        'm11', 'm22', 'm33', 'Xu', 'Xuu', 'Yv', 'Yvv', 'Yvr', 'Yr', 'Yrv', 'Yrr', 'Yur', 'Nv', 'Nvv', 'Nvr', 'Nr', 'Nrv', 'Nrr'
    ]
    with open("iacquabot_powell_identified_params.py", "w") as f:
        f.write("# Optimal dynamic parameters identified with Powell method (Symmetric)\n")
        f.write("import numpy as np\n\n")
        f.write("# --- DYNAMIC PARAMETERS ---\n")
        for name in param_names:
            f.write(f"{name} = {dynamic[name]:.10f}\n")
        f.write(f"\ntheta = [{', '.join([f'{dynamic[n]:.10f}' for n in param_names])}]\n\n")
    ...
    print("File 'iacquabot_powell_identified_params.py' successfully generated!")

    fig_evol, ax = plt.subplots(figsize=(8, 4.5))
    ax.plot(opt_history["dynamic_iterations"], opt_history["dynamic_costs"], "g-^", label="Dynamic Cost (Powell)")
    ...
    for idx, exp in enumerate(datasets):
        try:
            ...
            continue
    plt.tight_layout()
    unified_plot_path = "validation_powell_experiments.png"
    plt.savefig(unified_plot_path, dpi=150)
    plt.close(fig)
    print(f"\nPowell validation plot successfully saved to '{unified_plot_path}'!")
    with open("identified_dynamics_powell_metrics.json", "w") as f:
        json.dump(metrics_dict, f, indent=4)
    print("Metrics successfully saved to 'identified_dynamics_powell_metrics.json'!")
